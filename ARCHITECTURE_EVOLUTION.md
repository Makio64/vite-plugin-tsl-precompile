...
**Status (2026-05-13).** First no-behavior-change seams landed. [`packages/runtime/src/hydrate/texture-resolver.js`](packages/runtime/src/hydrate/texture-resolver.js) now owns uniform-plan texture lookup, shader texture-shape inference, texture-vs-binding compatibility checks, and shader-compatible fallback texture selection, with focused tests in [`packages/runtime/test/hydrate-texture-resolver.test.js`](packages/runtime/test/hydrate-texture-resolver.test.js). The `artifact.texture` path in [`hydrator.js`](packages/runtime/src/hydrator.js) is now split into named resolution strategies (`material-node-texture`, `render-target-texture-ref`, `live-texture-identity`, `texture-ref`, `material-slot-uuid`, `anonymous-data-texture`, `snapshot`, `multisampled-depth-fallback`, `anonymous-storage-texture`) and records the last strategy per binding on a non-enumerable `_textureResolutionStrategies` map.

**Next step.** Move the named `artifact.texture` strategies and their dependencies (`textureFromSnapshot`, `lookupLiveTextureByIdentity`, `lookupAnonymousDataTexture`, `lookupAnonymousStorageTexture`) out of `hydrator.js`, then surface resolution diagnostics through a documented debug hook instead of only artifact-local state.
