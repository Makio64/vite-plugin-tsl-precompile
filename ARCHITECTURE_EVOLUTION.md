...
**Status (2026-05-13).** Not done yet. The harness already behaves like option (A), and the latest MRT wedge makes that more explicit by resizing PassNode render targets to the active MRT descriptor and retargeting global `renderer.setMRT(...)` scenes to captured multi-output artifacts before WebGPU pipeline creation. Runtime/API docs still do not define this as a supported mode.
