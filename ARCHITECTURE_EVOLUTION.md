...
**Current read.** This roadmap is good to use, but it is not "done." The first shared-contract, graph-normalization, slim-support, texture-resolution, hydrator-rebinder, codegen-parse, coverage-config, and strict-rewrite wedges have landed. The unfinished evolution is the second half: move the rest of the harness runtime behavior into `slim-support`, finish shrinking `hydrator.js` into allocation/source/dynamic modules, turn dynamic binding descriptors into emitted/runtime-resolved artifact data, **make the production-preview pipeline work end-to-end on real-world examples** (the new §P1.8 — `vite build && vite preview` of `packages/examples/ocean` is currently a black canvas because of four layered gaps in `precompileAuxiliary` runtime behavior, aux-artifact registration, Inspector preview-mode, and slim-mode `Node` method-chain coverage), harden the three.js compat matrix, and add a dev-vs-build extractor convergence guard.
...
[`packages/runtime/src/slim-support/scene-support.js`](packages/runtime/src/slim-support/scene-support.js) is the public **`createSlimSceneSupport()`** orchestrator referenced at the top of this section — composes the five primitive modules into a single opt-in entry point (`indexScene`, `getFullRenderer`, `generatePMREMAsync`, `setPMREMGenerator`, `syncComputeOutputs`, `shareTexture`, `shareShadowTexture`, `dispose`), with a shared `diagnostics` bag namespaced `{ pmrem, textureShare, compute }` and an `onError(err, where)` for non-fatal sub-module failures. Unit-test coverage in [`packages/runtime/test/slim-support-scene-support.test.js`](packages/runtime/test/slim-support-scene-support.test.js); 9 cases covering the opt-in defaults, fallback boot, no-op compute sync, dispose tear-down, texture-share diagnostics, scene-index integration, and PMREM cache routing.
...
**Next step.** Migrate the harness `__getComputeRenderer` call sites onto `createFullRendererFallback` / `createSlimSceneSupport` so the harness is a thin caller of the public API on this axis too, then productize the remaining pass/shadow delegation (`__renderPassNodeWithFullRenderer`, the shadow scene cache + `__findVsmBlurTexture`) into the same orchestrator under `support.renderPassWithFallback(...)` / `support.populateShadowMap(...)`.
...
**Status (2026-05-13).** Productized and shipping. [`packages/runtime/src/slim-support/full-renderer-fallback.js`](packages/runtime/src/slim-support/full-renderer-fallback.js) owns the lazy-init + shared-device bootstrap + dispose. [`packages/runtime/src/slim-support/scene-support.js`](packages/runtime/src/slim-support/scene-support.js) composes it with `live-scene-index`, `pmrem`, `compute-sync`, and `gpu-texture-share` behind `createSlimSceneSupport`. [`packages/runtime/test/slim-support-full-renderer-fallback.test.js`](packages/runtime/test/slim-support-full-renderer-fallback.test.js) + [`slim-support-scene-support.test.js`](packages/runtime/test/slim-support-scene-support.test.js) cover the boot, dispose, device-sharing, and PMREM cache paths. `run-e2e.mjs`'s `__getComputeRenderer` now delegates to the productized fallback so the harness exercises exactly the same code path adopters will.
