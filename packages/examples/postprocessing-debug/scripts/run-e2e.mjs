...
const forwarded = [
	...
	'--save-shots',
	'--report=postprocessing-debug-e2e-report.json',
	...( hasOutputRoot ? [] : [ `--output-root=${ resolve( evidenceRoot, 'cohorts/postprocessing-debug' ) }` ] ),
	...args,
];
...
child.on( 'error', ( err ) => {
	console.error( `[postprocessing-debug-e2e] failed to start: ${ err.message }` );
	process.exit( 1 );
} );
