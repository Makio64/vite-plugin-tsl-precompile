...
const forwarded = [
	...
	'--save-shots',
	'--report=compute-debug-e2e-report.json',
	...( hasOutputRoot ? [] : [ `--output-root=${ resolve( evidenceRoot, 'cohorts/compute-debug' ) }` ] ),
	...args,
];
...
child.on( 'error', ( err ) => {
	console.error( `[compute-debug-e2e] failed to start: ${ err.message }` );
	process.exit( 1 );
} );
