...
const forwarded = [
	...
	'--save-shots',
	'--report=wow-showcase-e2e-report.json',
	...( hasOutputRoot ? [] : [ `--output-root=${ resolve( evidenceRoot, 'cohorts/wow-showcase' ) }` ] ),
	...args,
];
...
child.on( 'error', ( error ) => {
	...
	console.error( `[wow-showcase-e2e] failed to start: ${ error.message }` );
	process.exit( 1 );
