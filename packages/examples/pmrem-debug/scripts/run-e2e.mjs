...
const forwarded = [
	...
	'--save-shots',
	'--report=pmrem-debug-e2e-report.json',
	...( hasOutputRoot ? [] : [ `--output-root=${ resolve( evidenceRoot, 'cohorts/pmrem-debug' ) }` ] ),
	...args,
];
...
child.on( 'error', ( err ) => {
	console.error( `[pmrem-debug-e2e] failed to start: ${ err.message }` );
	process.exit( 1 );
} );
