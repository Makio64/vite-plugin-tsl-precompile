/**
 * IBL DFG LUT — precomputed, shared across all precompiled materials.
 *
 * Three.js bakes the DFG LUT as a static 16×16 RG16F DataTexture inside
 * `three/src/nodes/functions/BSDF/DFGLUT.js`. Because it is pure data (no
 * GPU compute required), the slim runtime can reconstruct the identical
 * DataTexture from the same bytes without needing a renderer. The texture is
 * created once, cached, and returned by `getDFGLUT()` for every artifact
 * that requests a `builtin.dfgLUT` binding.
 *
 * If the data is unavailable at import time (e.g. the three.js import path
 * shifted in a vendor bump), `getDFGLUT()` returns null and the hydrator
 * falls back to the 1×1 white fallback — IBL will look wrong but not crash.
 *
 * @module DFG_LUT
 */
...
/**
 * Return the shared precompiled IBL DFG LUT as a three.js DataTexture.
 * Created once and cached. Returns null if `DataTexture` is not importable
 * (should not happen in any normal runtime configuration).
 *
 * @return {import('three').DataTexture|null}
 */
export function getDFGLUT() {
	...
	try {
		...
		_lut = new DataTexture( DFG_DATA, 16, 16, RGFormat, HalfFloatType );
		_lut.name = 'DFG_LUT';
		_lut.minFilter = LinearFilter;
		_lut.magFilter = LinearFilter;
		_lut.wrapS = ClampToEdgeWrapping;
		_lut.wrapT = ClampToEdgeWrapping;
		_lut.generateMipmaps = false;
		_lut.needsUpdate = true;
