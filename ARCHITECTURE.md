...
- `src/three-rewrite.js` — strict, version-locked AST rewrites for the compiler-free Three closure. The r184 Renderer rewrite installs the exact-caster replay-material handoff and removes the now-redundant `_getShadowNodes()` cache/method/call plus its color/depth/position graph assignments. The exact `CubeRenderTarget.fromEquirectangularTexture()` lifecycle is also gated and its fixed TSL graph is replaced by a preflighted replay adapter before any caller texture mutation. Complete comment-free AST fingerprints replace Three's last `NodeUtils` / node-constants owners with four runtime primitives; unexpected module or export drift fails closed. Any upstream shape drift fails the slim build instead of partially applying a cut.
...
- `src/artifact-variants.js` — the shared variant-local payload field list and family merge contract used by capture, registries, codegen, and runtime. Private cache keys are only family-local: equivalent cross-family collisions canonical-union semantic selectors, divergent payload collisions fail closed instead of overwriting a shader family, and emitted family keys are ordered independently of capture arrival.
...
- `src/slim-source-entry.js` / `src/slim-source-common.js` — guarded application-tree-shaken slim mode. The Vite plugin routes Three internals through the same replay adapters as the checked prebuilt build, maps exact bare `three` imports to the consumer's `Three.Core.js` source graph, validates a plugin/runtime policy handshake, and rejects compiler or classified stock-adapter residue in final chunks. CI bundle budgets additionally lock the supported source fixtures to zero retained Node/TSL modules.
...
- `src/slim-support/auto-compute.js` — material-owned raw `ComputeNode` discovery, fail-closed writable-storage matching, retryable full-renderer bootstrap, and once-per-node dispatch. Assignments live on the material and are applied by the tiny `hydrate/material-compute-bindings.js` seam only after exact variant selection, so shared artifact families never retain another owner's GPU attribute. This hybrid adapter requires a retained real compute graph; pure stubbed-TSL source builds need a future build-time compute-ownership contract.
