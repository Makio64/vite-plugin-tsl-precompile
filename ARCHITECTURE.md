...
- `src/artifact-variants.js` — the shared variant-local payload field list and family merge contract used by capture, registries, codegen, and runtime. Private cache keys are only family-local: equivalent cross-family collisions canonical-union semantic selectors, while divergent payload collisions fail closed instead of overwriting a shader family.
